let btnGetWeather = document.querySelector("#submitBtn");

btnGetWeather.addEventListener('click', function search(event){
	event.preventDefault();
	let nameCity = document.getElementById('cityInput').value; 
	let errorInput = document.querySelector(".errorDisplay");
	if(nameCity == ""){
		errorInput = errorInput.textContent = "Enter Your City!"
	}
	// else if(typeof(nameCity).number){
	// 	errorInput = errorInput.textContent = "Enter correct City!"
	// }
	else{
		errorInput = errorInput.textContent = "";
	}

	let urlAddress = "http://api.openweathermap.org/data/2.5/weather";
	let unitsTemp = "metric";
	let appId = "59af365bd096b6cf99b18e3dd8c52136";
